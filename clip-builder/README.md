...
AI-powered short-form video builder: video analysis, scene tagging, and autonomous Instagram Reels generation. Brand-agnostic — each profile owns its own input/output folders under `~/Documents/ClipBuilder/`.
...
3. **Launch** — double-click `ClipBuilder.command` or run:
   ```bash
   source .venv/bin/activate
   python src/app.py
   ```
...
2. The app auto-creates two folders in your Drive: `ClipBuilder Inbox` and `ClipBuilder Output`.
...
3. **APIs & Services → OAuth consent screen**:
   ...
   - App name: `ClipBuilder`. Fill in support email + developer contact.
...
4. **APIs & Services → Credentials → + Create Credentials → OAuth client ID**:
   ...
   - Name: `ClipBuilder Local`.
   - **Authorized redirect URIs**: add `http://localhost:5555/drive/oauth/callback` (and `:8080` if you sometimes use that port).
   - Click **Create**, then **Download JSON**.
5. Save the downloaded file as `src/drive_client.json` in your ClipBuilder folder. (A template is at `src/drive_client.example.json` for reference.)
6. Restart the app.
