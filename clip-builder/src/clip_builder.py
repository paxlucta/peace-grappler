"""clip_builder.py — Clip Builder routes + UI for ClipBuilder."""
...
.tl-hdr .tot{color:#888;margin-left:auto}
/* Timeline zoom slider — drag right to shrink blocks (more of the
   timeline fits on screen); drag left to enlarge them. Sits inline in
   the timeline header. */
.tl-hdr .tl-zoom{
  display:inline-flex;align-items:center;gap:6px;margin-left:14px;
}
.tl-hdr .tl-zoom-ic{color:#666;font-size:14px;line-height:1}
.tl-hdr .tl-zoom-ic.small{font-size:10px}
.tl-hdr .tl-zoom input[type=range]{
...
.tl-hdr .tl-zoom input[type=range]::-webkit-slider-thumb{
  -webkit-appearance:none;appearance:none;width:14px;height:14px;
  border-radius:50%;background:#e53935;cursor:pointer;border:none;
}
.tl-hdr .tl-zoom input[type=range]::-moz-range-thumb{
...
<footer>
  <div class="tl-hdr">
    ...
    <div class="tl-zoom" title="Drag right to fit more of the timeline on screen">
      <span class="tl-zoom-ic" aria-hidden="true">⬚</span>
      <input type="range" id="tl-zoom" min="0" max="100" value="60"
             oninput="setTimelineZoom(this.value)">
      <span class="tl-zoom-ic small" aria-hidden="true">▭</span>
    </div>
...
// Timeline zoom — bumps CELL_W (the px-per-second the ruler/blocks use)
// and re-renders. Slider value 0..100 maps to CELL_W 120..20: dragging
// the slider RIGHT shrinks the blocks so more of the timeline is on
// screen at once. The slider position itself is persisted in
// localStorage so it survives reloads and tab switches.
var _TL_ZOOM_MIN_CELL = 20;
var _TL_ZOOM_MAX_CELL = 120;
var _TL_ZOOM_DEFAULT = 60;
var _TL_ZOOM_KEY = 'pg.builder.tlZoom';
function setTimelineZoom(sliderVal, opts) {
  var v = Math.max(0, Math.min(100, parseFloat(sliderVal) || 0));
  CELL_W = Math.round(_TL_ZOOM_MAX_CELL
    - (v / 100) * (_TL_ZOOM_MAX_CELL - _TL_ZOOM_MIN_CELL));
  try { localStorage.setItem(_TL_ZOOM_KEY, String(v)); } catch (e) {}
  // Keep the slider control in sync when this was called outside the
  // input event (e.g. on initial restore).
  if (!opts || !opts.skipInput) {
    var inp = document.getElementById('tl-zoom');
    if (inp && parseFloat(inp.value) !== v) inp.value = v;
  }
  if (typeof syncTl === 'function') syncTl();
}
function _restoreTimelineZoom() {
  var raw = null;
  try { raw = localStorage.getItem(_TL_ZOOM_KEY); } catch (e) {}
  var v = (raw === null || raw === '') ? _TL_ZOOM_DEFAULT : parseFloat(raw);
  if (!isFinite(v)) v = _TL_ZOOM_DEFAULT;
  setTimelineZoom(v);
}
// Restore on first paint — defer until DOM nodes exist.
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _restoreTimelineZoom);
} else {
  _restoreTimelineZoom();
}
