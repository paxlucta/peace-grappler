...
@settings_bp.route("/settings/api/app/new", methods=["POST"])
def api_app_new():
    """Create a new blank profile and switch to it. Body: {name: "MyBrand"}.

    Blank means: no content_domain, no tag_schema seeded, and source/output
    folders pointing at ~/Documents/ClipBuilder/<name>/{Input,Output} so each
    profile owns its own working tree out of the box.
    """
    data = request.get_json(force=True) or {}
    name = (data.get("name") or "").strip()
    try:
        created = app_config.create_blank_profile(name)
    except ValueError as e:
        return jsonify({"ok": False, "error": str(e)}), 400
    out = app_config.get_config()
    out["profiles"] = app_config.list_profiles()
    out["active_profile"] = app_config.get_active_profile_name()
    return jsonify({"ok": True, "created": created, **out})


@settings_bp.route("/settings/api/app/import", methods=["POST"])
def api_app_import():
    """Import a brand profile JSON: validate, copy into ~/Documents/ClipBuilder/, set
    active. Body shape:
...
<main>
  ...
  <p class="muted">
    ...
    <code>~/Documents/ClipBuilder/</code> that captures every setting on this page.
  ...
  <div class="card" style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
    <button class="prof-btn" onclick="openNewProfile()"
            title="Create a new brand profile — blank, or pre-filled by the Settings Wizard.">
      New
    </button>
    ...
    <span class="muted">Switch the active brand by loading its JSON file
      from <code>~/Documents/ClipBuilder/</code>, or start a new one.</span>
  </div>
  ...
  <div class="card">
    <div>
      ...
      <div class="muted" style="margin-top:4px">
        Saved as <code>~/Documents/ClipBuilder/&lt;name&gt;.json</code>. Saving with a
  ...
  <div id="wiz-overlay" class="wiz-overlay">
    <div class="wiz-modal">
      <h3 id="wiz-title" style="margin:0 0 8px 0;color:#fff">Settings Wizard</h3>
      <p class="muted" style="margin:0 0 14px 0">
        ...
        proposes a tag schema; you review &amp; click Save. Or skip the URL
        and create a blank profile to fill in by hand.
      </p>
      <label>Profile name <span id="wiz-name-req" class="muted" style="display:none">(required)</span></label>
      <input type="text" id="wiz-profile-name" placeholder="e.g. MyBrand">
      <label style="margin-top:12px">URL</label>
      ...
      <div style="margin-top:14px;display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap">
        ...
        <button id="wiz-blank" onclick="createBlankProfile()" class="prof-btn">Create Blank Profile</button>
...
function openWizard(){
  ...
  document.getElementById('wiz-profile-name').value = '';
  document.getElementById('wiz-title').textContent = 'Settings Wizard';
  document.getElementById('wiz-name-req').style.display = 'none';
  document.getElementById('wiz-overlay').dataset.mode = 'wizard';
  setWizStatus('', '');
  _populateWizModel();
...
function openNewProfile(){
  document.getElementById('wiz-url').value = '';
  document.getElementById('wiz-profile-name').value = '';
  document.getElementById('wiz-title').textContent = 'New Profile';
  document.getElementById('wiz-name-req').style.display = 'inline';
  document.getElementById('wiz-overlay').dataset.mode = 'new';
  setWizStatus('', '');
  _populateWizModel();
  document.getElementById('wiz-overlay').classList.add('active');
  setTimeout(() => document.getElementById('wiz-profile-name').focus(), 50);
}
...
async function createBlankProfile(){
  const name = (document.getElementById('wiz-profile-name').value || '').trim();
  if (!name) {
    setWizStatus('Profile name is required.', 'err');
    document.getElementById('wiz-profile-name').focus();
    return;
  }
  const btn = document.getElementById('wiz-blank');
  btn.disabled = true;
  setWizStatus('Creating blank profile "' + name + '"…', 'working');
  try {
    const r = await fetch('/settings/api/app/new', {
      method:'POST', headers:{'Content-Type':'application/json'},
    ...
    setWizStatus('Created. Reloading…', 'ok');
    setTimeout(function(){ location.reload(); }, 300);
  } catch (e) {
...
async function runWizard(){
  ...
  try {
    ...
    // Profile name precedence: explicit value typed in the modal wins;
    // otherwise follow extracted brand_name when the main field is empty
    // or still matches the previous brand.
    const modalName = (document.getElementById('wiz-profile-name').value || '').trim();
    const pn = document.getElementById('profile-name');
    if (modalName) {
      pn.value = modalName;
    } else if (ext.brand_name) {
